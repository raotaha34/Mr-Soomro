(function () {
  ...
  /* Chat API origin: same origin when the site is served over http(s), so the
     widget keeps working once deployed. Override with a <meta name="chat-api-base">
     tag when the API lives on another host. */
  var apiMeta = document.querySelector('meta[name="chat-api-base"]');
  var CHAT_API_BASE = (apiMeta && apiMeta.getAttribute("content")) ||
  ...
  if (!document.getElementById("chatbotBtn")) {
    ...
    var sendSharedMessage = async function () { 
      ...
      try {
        var response = await fetch(CHAT_API_BASE.replace(/\/$/, "") + "/api/chat", {
          method: "POST",
